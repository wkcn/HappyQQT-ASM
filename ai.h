...
uint8_t IsPassed(uint16_t x, uint16_t y){
	...
	if (PASSED_DATA[y * WinCol + x])return 0;
	if (STATE_DATA[y * WinCol + x])return 0;
	return 1;
}
...
void Update(){
	if (*PlayerNOHARM > 0)--*PlayerNOHARM;
		...
		if (STATE_DATA[y * WinCol + x] == PowerFlag){
			*PlayerNOHARM = AI_UpdateTimes * NOHARMTIME;
	...
	if (*BossNOHARM > 0)--*BossNOHARM;
		...
		uint16_t y = (r->y + 0x80) >> 8;
		if (STATE_DATA[y * WinCol + x] == PowerFlag){
			*BossNOHARM = AI_UpdateTimes * NOHARMTIME;
			if(*BossHP > 0)--*BossHP;
		}
	}
}
...
void AI(){
	...
	for (int r = 0;r < WinRow;++r){
		for (int c = 0;c < WinCol;++c){
			costMap[r][c] = 0;
			if (STATE_DATA[r * WinCol + c])costMap[r][c] += 5;
		}
	}
